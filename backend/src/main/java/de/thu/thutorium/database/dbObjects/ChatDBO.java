...
public class ChatDBO {
     ...
     * Todo : Review the cascade nature (All?)
     */
    @OneToMany(mappedBy = "chat", cascade = CascadeType.ALL, orphanRemoval = true)
    @Builder.Default
    private List<MessageDBO> chat_messages = new ArrayList<>();

    /**
     * The list of participants in the chat.
     *
     * <p>Defines a many-to-many relationship with {@link UserDBO}.
     * It is a bidirectional relationship(!!), the counterpart denoted as List<ChatDBO>
     *     called 'chats' in {@link UserDBO}.
     */
    @ManyToMany
    ...
    @Builder.Default
    private Set<UserDBO> chatParticipants = new HashSet<>();

    /**
     * Constructs a ChatDBO object with default values.
     */
    public ChatDBO() {
        this.chat_messages = new ArrayList<>();
        this.chatParticipants = new HashSet<>();
    }
}
